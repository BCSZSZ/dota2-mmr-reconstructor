...
- `队友报告包含普通匹配`：默认关闭；开启后，队友详情提取及报告合并天梯与常规普通匹配。
  MMR曲线和上分／下分仍仅用天梯。队友HTML可从分数图框选时间，结合普通匹配开关与最低局数重算八张Top 10图。
...
- 可行的隐藏区间通过有界端点约束分配整数 delta，命中下一真实 MMR。
- v0.5.6 起，端点变化超出模型范围的区间保留比赛记录，逐场分差留空、曲线断开，并显示真实端点；这些比赛不计入英雄和队友统计，报告会注明排除场数。不会把差额当作已确认的校准跳变。
